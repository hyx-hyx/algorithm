...
void solve() {
	...
	int cx, cy, cz, ans = 0;
	while (A.size()) {
		/*如果我们的狙击手的某一维度的坐标小于剩余点的那一维度的最小坐标，我们就应该把狙击手移动到那一维度的最小坐
		标，因为这样只会让我们能狙击的目标增多。在我们三个维度都是那一维度的最小坐标时，狙击手就必须将三个维
		度中某一维度最小坐标的所有敌军都消灭，才能继续移动。这时我们可以贪心的选取所需 k 最小的敌军消灭。*/
